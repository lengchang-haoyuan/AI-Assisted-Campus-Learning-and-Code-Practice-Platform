...
CREATE TABLE comments (
    ...
    CONSTRAINT fk_comments_user_id_users FOREIGN KEY (user_id)
        REFERENCES users (id) ON DELETE RESTRICT,
    CONSTRAINT ck_comments_comment_moderation_status CHECK (
        moderation_status IN ('visible', 'hidden')
    ),
    INDEX ix_comments_project_created (project_id, created_at),
    INDEX ix_comments_project_moderation_created (
...
CREATE TABLE notifications (
    ...
    community_publication_id BIGINT UNSIGNED NULL,
    community_case_id BIGINT UNSIGNED NULL,
    created_at DATETIME(6) NOT NULL,
    ...
    CONSTRAINT fk_notification_publication FOREIGN KEY (community_publication_id)
        REFERENCES community_publications (id) ON DELETE RESTRICT,
    CONSTRAINT fk_notification_governance_case FOREIGN KEY (community_case_id)
    ...
    CONSTRAINT ck_notifications_notification_target CHECK (
