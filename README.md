...
面向高校学生的 AI 辅助学习与代码实践平台。当前已完成 P00-P18 的设计与功能开发，覆盖认证、项目与校园代码社区、学习系统、可视化 Workflow、ProjectContext、AI Provider/Agent、Workflow 执行、真实数据统计、AI 学习报告、校园身份、教学班和教学任务。

Workflow 编辑器已经提供 AI 运行入口、历史结果和失败续跑，并增加解题提示、代码讲解、答案评审三个教学节点。P19-P24 仍是后续规划；真实教师确认、校方试点批准和真实班级试点均未发生。
...
每个阶段独立完成需求、验证、操作日志和完成报告。P00-P15 为原系统交付，P16 为校园试点设计，P17-P18 为已实现功能；P19-P24 只有在前置决策确认后实施。检查通过后创建对应 Git 提交并推送到 `origin/main`，确认远端提交哈希一致后再进入下一节点。

## 当前范围

前端 `/analytics` 使用 ECharts 展示数据库聚合的 7/30 日趋势、项目和技术栈统计，并可生成、刷新后查询当前用户的 AI 学习报告。校园身份与班级功能的验证明细见 [P17 验收报告](开发相关文档/阶段报告/20260905-P17-校园身份与账号管理验收.md) 和 [P18 验收报告](开发相关文档/阶段报告/20260907-P18-班级与教学任务验收.md)；启动和原系统演示路径见 [P15 演示与答辩指南](开发相关文档/答辩材料/P15演示与答辩指南.md)。依赖安全审计因外部服务超时仍未取得完整结论。
