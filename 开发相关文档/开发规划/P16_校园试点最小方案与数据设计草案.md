...
日期：2026-09-05，UTC+08:00。状态：**设计草案；未获用户设计确认、教师确认或校方试点批准，不可据此进入 P17。**
...
- 教师只可向本人有效班级邀请学生，不可授予教师或管理员。P17 先支持管理员校园邀请，教师班级邀请到 P18 增量加入 class_id。
...
| CampusInvitation/P17，班级字段 P18 | `issued_by_user_id`、`target_user_id?` 或 `target_email?`、`role student/teacher`、`token_digest BINARY(32)`、`expires_at`、`status pending/consumed/revoked/expired`、`consumed_by_user_id?`、`consumed_at?`；P18 加 `class_id?` | FK User、P18 FK TeachingClass；UNIQUE(token_digest)；INDEX(status,expires_at)、(issued_by_user_id,created_at)；目标二选一 CHECK；只有校园首次资格创建才关联邀请记录 |
...
| TeachingClass/P18 | `name VARCHAR(120)`、`course_title VARCHAR(120)`、`term_label VARCHAR(50)`、`status active/archived`、`created_by_user_id`、`archived_at?`、`revision` | FK creator→User；UNIQUE(term_label,name)；INDEX(status,created_at,id)；course_title 不 FK 到私人 Course |
| ClassMembership/P18 | `class_id`、`campus_membership_id`、`member_role student/teacher`、`status active/left/removed`、`joined_at`、`ended_at?`、`revision` | FK class/campus；UNIQUE(class_id,campus_membership_id)、UNIQUE(id,class_id)；INDEX(campus_membership_id,status,class_id)、(class_id,status,member_role)；member_role 必须与当前校园角色相容，管理员不能伪装教师 |
| TeachingAssignment/P18 | `class_id`、`created_by_membership_id`、`title VARCHAR(160)`、`instructions TEXT` 上限 8000 字符、`acceptance_criteria TEXT` 上限 4000、`due_at?`、`status draft/published/archived`、`published_at?`、`archived_at?`、`revision` | FK class；复合 FK(created_by_membership_id,class_id)→ClassMembership(id,class_id)；UNIQUE(id,class_id)；INDEX(class_id,status,due_at,id) |
...
| P17 `POST /campus/invitations` | target_user_id 或 target_email、role、expires_at；P18 加 class_id | 201 邀请元数据与一次性 token；管理员核验，教师仅本班学生；不返回已有邀请 token |
...
| P18 `GET/POST /campus/classes`；`PATCH /campus/classes/{classId}` | 分页；name/course_title/term_label；expected_revision/status | 管理员创建/安排，读取按班级作用域；不传私人 course_id |
| P18 `GET/POST /campus/classes/{classId}/members`；`PATCH /campus/classes/{classId}/members/{memberId}` | 已核验 campus_membership_id/member_role；expected_revision/status/reason | 管理员可安排教师，教师只能按邀请加入学生；学生不能任意加人；本人退出只允许改本人到 left |
| P18 `GET/POST /campus/classes/{classId}/assignments`；`PATCH /campus/assignments/{assignmentId}` | 标题、instructions、acceptance_criteria、due_at；expected_revision 和允许字段 | 教师建 draft；`POST /.../{id}/publish`、`POST /.../{id}/archive` 使用 expected_revision；专用 GET `/campus/assignments/{id}` 返回状态允许的内容 |
| P18 `DELETE /campus/assignments/{assignmentId}` | expected_revision（查询参数） | 204，仅本班教师删除无提交的 draft；已发布或归档返回 409 |
...
- P18：新增教学班、班级成员、任务；邀请追加可空 class_id。旧 Course.instructor 不做姓名匹配或教师回填；无真实名单不批量创建成员。
...
所有行当前为 **待确认**。用户在审阅后逐项注明“接受/修改”、确认人、日期、证据位置；未回复不是默认同意。

| ID | 事实与风险 | 选项与推荐 | 必须由谁确认/何时阻塞 |
| --- | --- | --- | --- |
| D1 | 现有账号没有校园资格；直接 role 回填会把旧账号当在校身份 | A 旧账号保留个人功能、校园邀请单独核验（推荐）；B 关闭所有普通注册，需明确兼容影响 | 用户确认方案；负责人/教师确认核验方式；P17 前必需 |
| D2 | 现有 JWT 不能按密码/角色变化撤销；加入版本会使旧会话失效 | A auth_version+一次重新登录（推荐）；B 新建持久化会话表，复杂度更高 | 用户确认字段、JWT/登录影响及切换方案；P17 前必需 |
| D3 | 尚无管理员/教师批准记录，不能默认升权 | 受控指定首管理员、管理员核验教师、教师仅本班邀请学生（推荐）；不设默认密码 | 负责人明确候选人与核验责任；用户批准初始化机制，真实执行另行授权；P17 前机制必需 |
| D4 | 邀请/重置/资格/审计均需新表和新 API | A 分阶段增量，无新增第三方依赖（推荐）；B 一次建全阶段表，拒绝作为默认 | 用户明确批准 P17 表字段/API DTO/新注册参数/审计范围；P17 前必需，不以本路线图代替批准 |
| D5 | 私人 Course 与教学任务生命周期不一致 | 独立班级与任务，发布后冻结题意，仅延长期限，归档终态（推荐）；更复杂任务版本暂缓 | 用户和真实教师；P18 前必需 |
| D6 | 共享整个 Project 会泄露 Context；取消社区发布不等于撤销教学证据 | 不可变白名单快照、一次反馈、退回再交、来源 SET NULL、通过只写无时长事件（推荐）；动态全项目共享不推荐 | 用户与教师确认边界、误评更正及截止规则；P19 前必需 |
...
- [ ] 用户逐项确认 D1–D4，确认 P17 新表/字段、API 草案、旧 JWT 重新登录、旧个人账号继续使用的兼容性方案。
- [ ] 确认固定三角色与私人数据隔离、管理员初始化和最后管理员保护、角色撤销的实际后果。
- [ ] 明确 P17 开发/验收只用隔离虚构账号，不把草案当作创建真实管理员或学生账号的授权。
- [ ] 对当前未提交的后续代码基线给出处理安排；P17 先核对差异，不能把它们误删或认为已随 P16 推送。
- [ ] 明确访问方案、AI 费用承担者和维护者；尚未准入的真实访问/模型外发保持关闭，不能由开发者代替校方批准。

确认层级记录：设计草案已完成仅表示文档可审查；用户设计确认、教师教学确认、校方批准试点分别留记录。目前后三项均未取得。本文件提交和推送也不改变这些状态。
