...
- 登录页沿用冷白、海沫绿和钴蓝基调，桌面采用内容与表单的双列布局；窄屏表单优先，展示内容在其后。
- 展示「项目实践」「学习工作台」「校园代码社区」三个主题，复用现有插画，不展示虚构统计。主题控件选中背景滑动，插画交叉过渡，首屏元素短时渐入。
- 滑块采用跟手填充、拖动时轻微形变、未完成回弹和通过后的勾选反馈，尺寸稳定；支持指针和键盘。失败、过期、刷新与提交中都有明确状态。
- 密码显隐可以通过键盘操作；`prefers-reduced-motion` 关闭位移和入场动画。动画不改变身份认证规则，后端仍是校验边界。
